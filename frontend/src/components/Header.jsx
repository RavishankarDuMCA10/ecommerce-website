        ...
        <div className="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
            <div className="flex item-center gap-x-2">
                {protected_routes.includes(pathname) && <>
                    <button className='text-2xl bg-gray-100 rounded-full p-2 hidden md:block' onClick={()=>dispatch(setCollapse())}>
                        <IoIosMenu />
                    </button>
                    <button className='text-2xl bg-gray-100 rounded-full p-2 block md:hidden' onClick={()=>dispatch(setToggle())}>
                        <IoIosMenu />
                    </button>
                    </>}
            </div>
